...
一个应用应该是一组小型服务；可以通过H3TTP的方式进行互通；
...
@**ImportResource**：如果想使用老Spring的xml配置，该注解可以导入Spring的配置文件，让配置文件里面的内容生效；
...
1、配置类**@Configuration**------>相当于以前的Spring配置文件
...
​	**1）、SpringBoot启动会加载大量的自动配置类**（jar包下META-INF/spring.factory文件中定义的）
...
每一个日志的实现框架都有自己的配置文件。使用slf4j以后，**配置文件还是做成日志实现框架自己本身的配置文件；** 因为最终是日志实现类使用配置文件。

### 2、遗留问题

a（slf4j+logback）: Spring（commons-logging）、Hibernate（jboss-logging）、MyBatis、xxxx

项目使用slf4j+logback，但是依赖的项目Spring使用commons-logging实现，hibernate使用joss-logging实现。。。
...
偷天换日，中间替换包，它里面的包名和它要替换的日志框架保持一致，但是实现却偷偷的使用slf4j的API。
