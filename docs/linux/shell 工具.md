...
2. `stdbuf -o0 numactl --cpunodebind=netdev:$port0 --membind=netdev:$port0 iperf -s -p 16003 -i 2 >> "$port0_rx" &`

   强制进程：

   - CPU 核心和网卡在同一个 NUMA 节点（避免跨节点调度）；

   - 内存分配在网卡所在节点的本地内存（避免远程内存访问）；

     最终大幅降低网络数据包处理的延迟、提升吞吐量（对 DPDK/SPDK/ 高性能转发程序，性能提升可达 30%+）。
