...
<font color=red>遍历框架本身是一个递归函数,只是当问题只是打印节点值时不需要返回值！  一旦我们的问题需要有返回值，我们也可以让这个遍历框架中的递归函数有返回值，这个返回值就是问题的答案，递归函数的每次调用就是解决一个子问题。而后序位置可以通过子问题的答案得到父问题的答案,  重点是思考当前节点该做什么。   其次，递归函数也可以视具体问题带有参数</font>
...
>
> 中序位置，当前节点，能接收到参数和一个子问题的信息
>
> 后序位置，向上离开某个节点，除了参数和节点信息外，还可以知道两个子问题的答案(遍历2的时候，两个红色箭头)
...
<font color=red>分治思想下，通常用递归函数解决问题，考虑到递归框架本身也是一个递归函数，只是这个递归函数返回了空值, 它没有要解的问题，也不需要组织答案。我们是需要答案的，首先这个递归函数需要有返回值，其次，要思考如何组织子问题的答案得到原问题的答案；也就是在后序位置组织答案。对遍历框架中的递归函数进行修改即可。</font>
...
2. <font color=red>**修改递归函数，使其返回子问题答案，在后序位置中组织原问题答案**</font>
