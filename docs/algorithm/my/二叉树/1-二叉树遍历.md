...
<font color=red>2.递归函数返回值：考虑到递归框架本身也是一个递归函数，只是这个递归函数返回了空值, 它没有要解的问题，也不需要组织答案。我们是需要答案的，首先这个递归函数需要有返回值，返回值就是跟当前节点相关的子答案，也就是在后序位置组织子答案。3. 其次，要思考如何组织子问题的答案得到原问题的答案；例如利用全局变量记录</font>
