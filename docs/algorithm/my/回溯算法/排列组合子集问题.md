...
剪枝思路：先排序，让相同的元素靠在一起，遍历所有选项时，相同选项只遍历一个即可。
...
这里的剪枝比较难以想到，如果把重复数字的顺序固定，那么它们的排列就只有一个了。假如重复元素说：我只跟着前面的大哥走 ，前面的大哥如果没在路径里，就不能选择我，从而达到剪枝。
...
1. 组合/子集问题因为不可复选，且不考虑元素顺序，所以可以用start来剪枝（只能从其余元素里选）。
2. 排列问题是考虑顺序的， 不同顺序是不同的排列，所以可以选择前面的元素，不能用start剪枝，只能用used数组来剪枝排除掉已在路径中的元素。
