...
广播变量允许程序员在每台机器上缓存一个只读的变量，而不是每个task保存一份拷贝。这是把大数据集拷贝到每个节点的一种高效的方式。spark也尝试使用广播算法减少分发广播变量的传输开销

### 问题：为什么只能 broadcast 只读的变量？

这就涉及一致性的问题，如果变量可以被更新，那么一旦变量被某个节点更新，其他节点要不要一块更新？如果多个节点同时在更新，更新顺序是什么？怎么做同步？还会涉及 fault-tolerance 的问题。<font color=red>为了避免维护数据一致性问题，Spark 目前只支持 broadcast 只读变量。</font>
...
![TorrentBroadcast](https://piggo-picture.oss-cn-hangzhou.aliyuncs.com/TorrentBroadcast.png)
