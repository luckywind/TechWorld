...
1. 执行计划总不是最优的
   执行计划在运行期间无法更改，比如两个大表Join,其中一个在过滤后可以广播，但执行计划仍然使用sort merge join，造成大量shuffle
2. 倾斜key
   一般我们通过手动处理数据倾斜，费时费力。
...
什么时候优化？各stage之间RDD需要物化(map阶段)，这样可以知道每个中间结果分区的大小，且后续task还没开始，这里是重新优化的机会。
...
例如，<font color=red>AQE在运行时发现tb2实际大小/经过过滤后比估计的小，从而把sort mege join策略修正为broadcast join策略</font>
