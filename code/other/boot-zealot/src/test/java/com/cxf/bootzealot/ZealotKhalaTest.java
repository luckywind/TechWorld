...
public class ZealotKhalaTest {
    ...
    public void testSelect() {
        SqlInfo sqlInfo = ZealotKhala.start()
                ...
                /**
                 * text方法有两个重载方法，
                 * 1。 text(String text, Object... values)
                 * 拼接text字符串，并对text的SQL字符串中绑定变量追加有序参数，
                 * values是任意类型的不定参数，即可以传递数组或任意个数的变量，用来作为该SQL片段的有序参数
                 * 2。 text(boolean match, String text, Object... values)
                 * match: 是否生成该SQL片段及有序参数
                 */
                .text(true, "AND u.email = ?", "san@163.com") //这里加了一个有序参数
                .doAnything(true, new ICustomAction() {
                    @Override
                    public void execute(StringBuilder join, List<Object> params) {
                        join.append(" hi");
                        params.add(5);      //这里加了一个有序参数，但可以不使用
                        log.info("执行了自定义操作，可拼接字符串和有序参数...");
                    }
                })
                .groupBy("u.id").having("u.id")
                .orderBy("u.id").desc().text(", u.nick_name", "zhang").asc()//这里加了一个有序参数，但可以不使用
                .unionAll()
                .select("u.id, u.nick_name, u.email")
                .from("user2")
                .limit("5")
